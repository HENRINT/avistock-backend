...
public class CajaReporteController {
    ...
    public void obtenerNotificacionesPendientes(Context ctx) {
        try (EntityManager em = emf.createEntityManager()) {
            ...
            // CORREGIDO: esta consulta trae UNA sola columna (mensaje), así que Hibernate
            // devuelve una lista de String directamente, no de Object[] — declararlo como
            // Object[] causaba "String cannot be cast to Object[]" y tronaba el endpoint
            // completo (por eso nunca cargaba nada, ni para el cajero ni para el dueño).
            ...
            respuesta.put("mensaje", corte.isEmpty() ? "" : corte.get(0).toString());
    ...
    public void generarYEnviarReporteEnVivo(Context ctx) {
        ...
        try {
            ...
            em.createNativeQuery("INSERT INTO notificaciones_caja (tipo, mensaje, id_cierre, leida, fecha_creacion, detalle_texto) VALUES ('REPORTE_ENVIVO', ?, ?, false, NOW(), ?)")
                    ...
                    .executeUpdate();
            // NUEVO: marca la solicitud del dueño como atendida — sin esto, el botón
            // "Generar Reporte" del cajero se quedaría habilitado para siempre después
            // de cumplir la solicitud, en vez de volver a bloquearse hasta la próxima.
            em.createNativeQuery("UPDATE notificaciones_caja SET leida = true WHERE tipo = 'CORTE_SOLICITADO' AND leida = false")
                    .executeUpdate();
            em.getTransaction().commit();
